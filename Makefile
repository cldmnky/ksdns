...
# ksdns.io/ksdns-bundle:$VERSION and ksdns.io/ksdns-catalog:$VERSION.
IMAGE_TAG_BASE ?= ksdns.io/ksdns
...
IMG_KSDNS ?= quay.io/ksdns/ksdns-operator:latest
...
# More info: https://docs.docker.com/develoS}

.PHONY: build-ksdns-release
build-ksdns-release: gox generate fmt vet ## Build zupd binary.
	${GOX} -osarch=${RELEASE_IMAGE_PLATFORMS} -output="bin/release/{{.OS}}/{{.Arch}}/ksdns-operator"

.PHONY: build-ksdns-image
build-ksdns-image:  build-ksdns-release ## Build docker image with zupd.
	podman build -t ${IMG_KSDNS} -f Dockerfile.ksdns-operator bin/release
...
multiarch-image-ksdns: build-ksdns-release ## Build multiarch container image with ksdns-operator.
	...
	podman manifest exists ${IMG_KSDNS} || podman manifest create ${IMG_KSDNS}; \
